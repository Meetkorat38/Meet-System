          ...
          {p.sampleOutputs && p.sampleOutputs.length > 0 && (
            <Section title="Automation outputs">
              <p className="mb-6 text-sm text-subtle-foreground leading-relaxed max-w-2xl">
                Real videos generated by the pipeline from a topic prompt —
                play any short below.
              </p>
              <div className="space-y-10">
                {Object.entries(
                  p.sampleOutputs.reduce<
                    Record<string, typeof p.sampleOutputs>
                  >((acc, item) => {
                    const key = item.series ?? "Outputs";
                    (acc[key] ??= []).push(item);
                    return acc;
                  }, {}),
                ).map(([series, items]) => (
                  <div key={series}>
                    ...
                    <div className="grid sm:grid-cols-2 gap-5">
                      {items!.map((item) => (
                        ...
                        >
                          <div className="aspect-[9/16] max-h-[420px] bg-black sm:max-h-[480px]">
                            <video
                          ...
                          <div className="border-t border-border px-4 py-3">
                            ...
                            {item.description && (
                              <p className="mt-1 text-xs text-subtle-foreground leading-relaxed">
                                {item.description}
                              </p>
                            )}
                          </div>
                ...
                {p.gallery.map((img) => (
                  ...
                  >
                    <img
                      ...
                      loading="lazy"
                    />
                    <figcaption className="border-t border-border px-4 py-3 font-mono text-[11px] text-muted-foreground">
                      {img.label}
                    </figcaption>
                  </figure>
                ))}
